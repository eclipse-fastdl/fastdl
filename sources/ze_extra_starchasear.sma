...
// Models
new g_szVModel[128] = "models/ozzy_extras/v_starchaserar.mdl"
new g_szWModel[128] = "models/csnz/w_starchaserar.mdl"
new g_szPModel[128] = "models/csnz/p_starchaserar.mdl"
...
#define WEAPON_SHOOT_DAMAGE	50.0
#define WEAPON_EXPLODE_DAMAGE	100.0
...
#define MUZZLEFLASH_INTERVAL 0.06
#define STAR_EFFECT_INTERVAL 0.10
#define STAR_EFFECT_FLAGS (TE_EXPLFLAG_NODLIGHTS | TE_EXPLFLAG_NOSOUND | TE_EXPLFLAG_NOPARTICLES)
...
new g_iItemID
new Float:g_flNextMuzzleFlash[33]
new const ZE_ITEM_NAME[] = "StarChaseAR"
new const ZE_ITEM_COST = 120
new const ZE_ITEM_LIMIT = 1
...
public plugin_cfg()
{
	if (!ini_read_string(ZE_ET_FILENAME, ZE_ITEM_NAME, "VIEW_MODEL", g_szVModel, charsmax(g_szVModel)))
		ini_write_string(ZE_ET_FILENAME, ZE_ITEM_NAME, "VIEW_MODEL", g_szVModel)
	if (!ini_read_string(ZE_ET_FILENAME, ZE_ITEM_NAME, "WEAPON_MODEL", g_szPModel, charsmax(g_szPModel)))
		ini_write_string(ZE_ET_FILENAME, ZE_ITEM_NAME, "WEAPON_MODEL", g_szPModel)
	if (!ini_read_string(ZE_ET_FILENAME, ZE_ITEM_NAME, "WORLD_MODEL", g_szWModel, charsmax(g_szWModel)))
		ini_write_string(ZE_ET_FILENAME, ZE_ITEM_NAME, "WORLD_MODEL", g_szWModel)
}
...
public client_disconnected(iPlayer, bool:drop, message[], maxlen)
{
...
{
	register_native("ze_star_chaser", "Get_MyWeapon", 1)
}
...
public ze_select_item_post(iPlayer, itemid, bool:bIgnoreCost)
{
	if (itemid != g_iItemID)
		return
...
public fw_SetModel(iEntity) <WeaponBox: Enabled>
{
	...
	for(new i, iItem; i < MAX_ITEM_TYPES; i++)
	{
		...
		{
			engfunc(EngFunc_SetModel, iEntity, g_szWModel)
			return FMRES_SUPERCEDE
		}
	}
...
public fw_Item_Deploy_Post(iItem)
{
	...
	set_pev(iPlayer, pev_viewmodel2, g_szVModel)
	set_pev(iPlayer, pev_weaponmodel2, g_szPModel)
...
Make_MuzzleFlash(iPlayer)
{
	...
	if (g_flNextMuzzleFlash[iPlayer] > flGameTime)
		return

	g_flNextMuzzleFlash[iPlayer] = flGameTime + MUZZLEFLASH_INTERVAL

	static Ent; Ent = engfunc(EngFunc_CreateNamedEntity, g_iInfoTarget)
...
public fw_Ball_Think(Ent)
{
	...
	static Float:flGameTime, Float:flNextEffect
	flGameTime = get_gametime()
	pev(Ent, pev_fuser1, flNextEffect)

	if(flNextEffect <= flGameTime)
	{
		...
		write_byte(STAR_EFFECT_FLAGS)
		message_end()

		set_pev(Ent, pev_fuser1, flGameTime + STAR_EFFECT_INTERVAL)
	}
...
public Safety_Connected(iPlayer)
{
	...
	g_flNextMuzzleFlash[iPlayer] = 0.0
}
...
Weapon_Create(Float: Origin[3] = {0.0, 0.0, 0.0}, Float: Angles[3] = {0.0, 0.0, 0.0})
{
	...
	engfunc(EngFunc_SetModel, iWeapon, g_szWModel)
