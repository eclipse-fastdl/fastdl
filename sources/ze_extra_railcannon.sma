...
// Marks the weapon ENTITY itself (not just the player), the same way
// ze_extra_cyclone / plasmagun / starchaser identify their custom weapons.
// This keeps working correctly even after the weapon changes owner (drop/pickup/death),
// which is what the old per-player-only flag could not guarantee.
#define IsRailcannonEnt(%1) (pev_valid(%1) && pev(%1, pev_impulse) == RAILCANNON_WEAPONKEY)
...
const Float:damage_mode3 = 150.0
...
stock const ZE_ITEM_NAME[] = "Rail Cannon"
stock const ZE_ITEM_COST = 150
...
public plugin_init()
{
	...
	// Reads the actual weapon stored in the weaponbox instead of the player's
	// current inventory (which is what made the w_ model detection unreliable,
	// e.g. right after death/drop the weapon is no longer in the player's inventory).
	...
	RegisterHam(Ham_RemovePlayerItem, "weapon_m3", "fw_RAILCANNON_RemovePlayerItem_Post", 1)
	RegisterHam(Ham_Item_Kill, "weapon_m3", "fw_RAILCANNON_ItemKill_Pre")
	RegisterHam(Ham_Spawn, "weapon_m3", "fw_RAILCANNON_Weapon_Spawn_Post", 1)
	...
	g_rail = ze_item_register(ZE_ITEM_NAME, ZE_ITEM_COST, ZE_ITEM_LIMIT)

}

public plugin_cfg()
{
	if (!ini_read_string(ZE_ET_FILENAME, ZE_ITEM_NAME, "VIEW_MODEL", RAILCANNON_V_MODEL, charsmax(RAILCANNON_V_MODEL)))
		ini_write_string(ZE_ET_FILENAME, ZE_ITEM_NAME, "VIEW_MODEL", RAILCANNON_V_MODEL)
	if (!ini_read_string(ZE_ET_FILENAME, ZE_ITEM_NAME, "WEAPON_MODEL", RAILCANNON_P_MODEL, charsmax(RAILCANNON_P_MODEL)))
		ini_write_string(ZE_ET_FILENAME, ZE_ITEM_NAME, "WEAPON_MODEL", RAILCANNON_P_MODEL)
	if (!ini_read_string(ZE_ET_FILENAME, ZE_ITEM_NAME, "WORLD_MODEL", RAILCANNON_W_MODEL, charsmax(RAILCANNON_W_MODEL)))
		ini_write_string(ZE_ET_FILENAME, ZE_ITEM_NAME, "WORLD_MODEL", RAILCANNON_W_MODEL)
}

public plugin_precache()
{
	precache_model("models/ozzy_extras/v_railcannon.mdl")
...
{
	register_native("ze_rail_cannon", "give_railcannon", 1)
}
...
public ze_select_item_post(player, itemid, bool:bIgnoreCost)
{
    if (itemid != g_rail)
        return
...
public fw_TraceAttack(iEnt, iAttacker, Float:flDamage, Float:fDir[3], ptr, iDamageType)
{
	...
	if(railcannon_mode[iAttacker] == 3)
	{
		...
		if(ze_is_user_zombie(iEnt) && g_KnockBack == 1) knockback(iEnt, myOrigin, 10.0, 2)
	}
...
public fake_smokes(id, Float:Origin[3])
{
	...
	set_pev(ent_test, pev_classname, SMOKE_CLASS)
...
public fw_WeaponBox_SetModel_Pre(const iEnt, const szModel[])
{
	...
	// Read the weapon that was actually packed into this weaponbox (works on
	// both manual drop and death), instead of trying to find it in the
	// player's inventory afterward, which no longer contains it by then.
	new iWeapon = get_member(iEnt, m_WeaponBox_rgpPlayerItems, PRIMARY_WEAPON_SLOT)

	if (IsRailcannonEnt(iWeapon))
		SetHookChainArg(2, ATYPE_STRING, RAILCANNON_W_MODEL)
}

stock SendWeaponListMsg(id, const name[])
{
...
{
	drop_weapons(id, 1)
	new iWep2 = rg_give_item(id, "weapon_m3", GT_APPEND)
	if( iWep2 > 0 )
	{
		// Mark the weapon entity itself, not just the player-side flag. This is
		// what makes the item correctly recognized (world model on drop, no
		// mix-up with a plain m3) even after being re-given on a new round.
		set_pev(iWep2, pev_impulse, RAILCANNON_WEAPONKEY)
		set_pdata_int(iWep2, OFFSET_CLIPAMMO, cvar_clip_railcannon, OFFSET_LINUX_WEAPONS)
		...
		g_clip_ammo[id] = get_pdata_int(iWep2, OFFSET_CLIPAMMO, OFFSET_LINUX_WEAPONS)
	}
	...
	SendWeaponListMsg(id, "weapon_railcannon")
}

public fw_RAILCANNON_AddToPlayer(railcannon, id)
{
	if(!pev_valid(railcannon) || !is_user_connected(id))
		return HAM_IGNORED
	
	if(IsRailcannonEnt(railcannon))
	{
		g_has_railcannon[id] = true
		
		SendWeaponListMsg(id, "weapon_railcannon")
		
		// NOTE: the impulse marker is intentionally kept on the entity (not
		// cleared) so the world model (w_) keeps being recognized correctly
		// every time this specific weapon is dropped again.
	...
	// A regular (non-marked) weapon_m3 was picked up instead, e.g. after
	// dropping the railcannon and grabbing a normal shotgun off the ground.
	// Make sure the stale flag doesn't leak railcannon behavior onto it, so
	// it coexists correctly with the base m3 weapon.
	g_has_railcannon[id] = false

	// The client's HUD caches the weapon icon by weapon ID, not by entity.
	// Once "weapon_railcannon" was ever sent for this ID, the game keeps
	// showing that icon for every future m3 until something re-registers
	// the default one. Force that correction here.
	SendWeaponListMsg(id, "weapon_m3")

	return HAM_IGNORED
}

public fw_RAILCANNON_RemovePlayerItem_Post(railcannon, id)
{
	if(!IsRailcannonEnt(railcannon))
		return

	// The weapon just left the player's hands (dropped or died with it).
	// Only the per-player state is cleared here; the entity keeps its
	// marker so the weaponbox world-model hook above still recognizes it.
	g_has_railcannon[id] = false
	update_specialammo(id, 0, 0)
	g_SpecialAmmo[id] = 0
}

public fw_RAILCANNON_ItemKill_Pre(railcannon)
{
	// The weapon entity is actually being destroyed now (e.g. removed when
	// its owner is infected/stripped of weapons, or left on the ground long
	// enough to expire). Its edict slot can be recycled by the engine for a
	// brand new weapon right after this, and nothing guarantees pev_impulse
	// gets zeroed out in that process. Clear the marker here so a later,
	// completely unrelated weapon_m3 (e.g. bought normally) never inherits
	// it and wrongly shows the railcannon HUD/behavior.
	if(IsRailcannonEnt(railcannon))
		set_pev(railcannon, pev_impulse, 0)
}

public fw_RAILCANNON_Weapon_Spawn_Post(weapon)
{
	// Defense in depth against edict recycling: no matter *how* a previous
	// railcannon entity was destroyed (Kill(), a direct engine removal, or
	// anything the framework does that our Ham_Item_Kill hook might miss),
	// every single weapon_m3 that comes into existence starts here with the
	// marker forcibly cleared. give_railcannon() re-marks it explicitly right
	// after this runs (rg_give_item spawns the entity before returning it),
	// so this can never wipe a legitimate railcannon by mistake — it only
	// ever prevents a stale marker from leaking onto an unrelated, freshly
	// bought m3.
	if(pev_valid(weapon))
...
public fw_Item_Deploy_Post(weapon_ent)
{
	...
	static weaponid
	weaponid = get_pdata_int(weapon_ent, 43, OFFSET_LINUX_WEAPONS)
...
replace_weapon_models(id, weaponid)
{
	switch (weaponid)
	{
		case CSW_M3:
		{
			...
			else if(oldweap[id] != CSW_M3)
			{
				// Same HUD-cache correction as in fw_RAILCANNON_AddToPlayer,
				// kept here too as a safety net for any m3 that reaches the
				// player through a path that doesn't fire Ham_Item_AddToPlayer.
				SendWeaponListMsg(id, "weapon_m3")
			}
		}
	}
...
public fw_RAIL_PrimaryAttack(Weapon)
{
	...
	g_clip_ammo[Player] = get_pdata_int(Weapon, OFFSET_CLIPAMMO, OFFSET_LINUX_WEAPONS)
...
public fw_CmdStart(id, uc_handle, seed) 
{
	...
	else if(CurButton & IN_ATTACK2)
	{
		if(!zz[id] && get_pdata_float(id, 83, 4) <= 0.0 && g_clip_ammo[id] >= 1)
		{
			...
			set_pdata_int(weapon, OFFSET_CLIPAMMO, g_clip_ammo[id] - 1, OFFSET_LINUX_WEAPONS)
			g_clip_ammo[id] --
		}
	}
...
public isi(id)
{
	...
	new clip, pluru, pengurang, bpammo, weapon = fm_get_user_weapon_entity(id, CSW_M3)
	get_user_weapon(id, clip, bpammo )
	pengurang = cvar_clip_railcannon - clip
	pluru = bpammo - pengurang
	cs_set_user_bpammo( id, CSW_M3, pluru )
	
	if(bpammo <= pengurang) set_pdata_int(weapon, OFFSET_CLIPAMMO, clip + bpammo, OFFSET_LINUX_WEAPONS)
	else set_pdata_int(weapon, OFFSET_CLIPAMMO, cvar_clip_railcannon, OFFSET_LINUX_WEAPONS)
...
{ 
	...
	vfEnd[0] = float(viEnd[0]); vfEnd[1] = float(viEnd[1]); vfEnd[2] = float(viEnd[2]) 
